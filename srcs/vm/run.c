...
/*   Updated: 2020/07/02 16:19:17 by scoron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		del_proc(t_cw_proc *dead_proc)
{
	...
	if (g_cwdata.verbose == 1)
		ft_printf("\nCycle %d : Process number %d (associated to %s (%d)) "
			"worthless life has ended\n", g_cwdata.nbr_cycle, dead_proc->nb,
			dead_proc->champ->name, dead_proc->champ->nb);
	while (current)
...
/*
 ** When we reach cycle_to_die, this function check who's alive and who dies.
 ** It calls process deletion when dead.
*/
...
void		cw_run(void)
{
	...
	while (g_cwdata.cycles_to_die > 0 && g_cwdata.nbr_proc)
	{
		...
		++g_cwdata.nbr_cycle;
		if (g_cwdata.verbose > 10 && g_cwdata.nbr_cycle == g_cwdata.verbose)
