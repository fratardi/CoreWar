...
t_int32		cw_lldi(t_cw_proc *proc, t_uint8 *pos)
{
	...
	arg[0] = cw_get_arg(proc, &cursor, 0, A_IND | A_DIR | A_VAL);
	arg[1] = cw_get_arg(proc, &cursor, 1, A_IND | A_DIR);
	rg = cw_get_arg(proc, &cursor, 2, A_REG);
	proc->pos = cw_jump(proc->pos, (t_int32)(cursor - proc->pos), 42);
	if (rg < 0x1 || rg > REG_NUMBER || cw_check_regs_valid(pos, 0xfc, 1))
		return (FAILED);
	to_read = cw_jump(pos, cw_sum(arg[0], arg[1]), 0);
	proc->reg[rg] = cw_read(to_read, sizeof(proc->reg[1]));
	if (g_cwdata.verbose == 1)
		ft_printf("lldi : copying value 0x%x (from addr distant of 0x%.4x + "
			"0x%.4x = 0x%.4x) to r%d\n", proc->reg[rg], arg[0], arg[1],
			cw_sum(arg[0], arg[1]), rg);
